...
const SuiviCard = ({ href, className, id }: SuiviCardProps) => {
    const suivi = suivis.find(suivi => suivi.id === id);
    {
        return (
            <Link href={href} className={`${className} relative bg-white md:aspect-square rounded-3xl overflow-hidden shadow-lg`}>
                ...
                <p className="mt-2 ml-4 font-bold">{suivi?.title}</p>
                <p className="text-center text-sm p-2 opacity-[40%]">{suivi?.contenu}</p>
            </Link>
        )
    }
}
